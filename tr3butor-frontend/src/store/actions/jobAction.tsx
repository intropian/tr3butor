...
export const getDaoJobs = (id: string) => {
  return async (dispatch: Dispatch<jobAction>): Promise<void> => {
    try {
      ...
      const response = await axios.get(`/api/dao/${id}/jobs?populate=1`)
      dispatch({
...
export const getJobs = () => {
  return async (dispatch: Dispatch<jobAction>): Promise<void> => {
    try {
      ...
      const response = await axios.get('/api/dao-job?populate=1')
      dispatch({
...
export const getJob = (id: string) => {
  return async (dispatch: Dispatch<jobAction>): Promise<void> => {
    try {
      ...
      const response = await axios.get(`/api/dao-job/${id}?populate=1`)
      dispatch({
